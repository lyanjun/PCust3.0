...
import com.c3.library.utils.BitmapFillet;
import com.c3.library.utils.DensityUtils;
...
public class VerifyCodeView extends View {
    ...
    private void initAttrValues(Context context, AttributeSet attrs){
        ...
        for (int i=0;i<count;i++){
            ...
            if (index == R.styleable.VerifyCodeView_codeTextSize) {// 默认设置为16sp，TypeValue类 px转sp 一个转换类
                mCodeTextSize = typedArray.getDimensionPixelSize(index, (int) TypedValue.applyDimension(
                        TypedValue.COMPLEX_UNIT_SP, 16, getResources().getDisplayMetrics()));

            } else if (index == R.styleable.VerifyCodeView_codeBackground) {
                mCodeBackground = typedArray.getColor(index, Color.WHITE);

            } else if (index == R.styleable.VerifyCodeView_codeLength) {
                mCodeLength = typedArray.getInteger(index, 4);

            } else if (index == R.styleable.VerifyCodeView_isContainChar) {
                isContainChar = typedArray.getBoolean(index, false);

            } else if (index == R.styleable.VerifyCodeView_pointNum) {
                mPointNum = typedArray.getInteger(index, 100);

            } else if (index == R.styleable.VerifyCodeView_linNum) {
...
//        UNSPECIFIED：表示子布局想要多大就多大，很少使用
    ...
    private Bitmap createBitmapValidate(){
        ...
        for (int i = 1; i <= length; i++) {
            ...
            //设置字体的绘制位置
            canvas.drawText(String.valueOf(mCodeText.charAt(i - 1)), mWidth/length*(i-1) + mWidth/length/2,
                    baseLineY, mPaint);
        ...
        sourceBitmap = BitmapFillet.fillet(sourceBitmap, DensityUtils.dp2px(getContext(),5f),BitmapFillet.CORNER_ALL);
        return sourceBitmap;
    }
